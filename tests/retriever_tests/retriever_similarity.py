...
from agent.retriever import retrieve_similarity_documents

from tests.retriever_tests.retrieval_output_common import (
    TEST_CASES,
    build_summary,
    case_passed,
    format_output,
...
def run_case(case):
    documents = retrieve_similarity_documents(
    ...
    report["log_path"] = save_report(report, prefix=f"{case['name']}_similarity")
    return report
